	...
	return function(app) {
		app.run(['$rootScope', '$state', '$stateParams','$location',function($rootScope, $state, $stateParams,$location) {
			...
			defaultErrorMessageResolver.getErrorMessages().then(function (errorMessages) {
          		...
          		errorMessages['recommendError'] = "请输入4-16位字符,由英文字母、数字组成";
          		...
          		errorMessages['MonthInteger'] = "请输入一个不超过36的月份数";
							...
							errorMessages['remote-friend'] = "好友推荐人不存在";
							errorMessages['minUser'] = "用户名不能小于4位";
				errorMessages['invalidcharacter'] = "存在非法字符!";
        	});
          ...
          attrs.activeClass = attrs.activeClass || 'active';
			...
			return {
				...
				link: function(scope, elm, attrs){
					...
					function eventBind (){
						// 初始化active条目
						// @DESC：当activeType已定义，并且switchType == activeType时，active条目
						...
						// 作用不明
						// @DESC：如果存在switchInit，且当switchType == activeType时，active条目，并调用switchAction
						if(attrs.switchInit){
							setTimeout(function(){
								angular.forEach($switchs,function(item){
									var $item = angular.element(item);
									if ($item.attr(attrs.switchType) == scope.activeType){
										$switchs.removeClass(attrs.activeClass);
										$item.addClass(attrs.activeClass);
										scope.switchAction({switchType:attrs.switchInit});
									}
								})
