define([
	...
	return function(app) {
		...
		}]).directive('countdown', ['$interval','cService', function($interval,cService) {
			return {
				link: function(scope, elm, attrs) {
					var count;
					...
					scope.$watch(attrs.countdown, function() {
						...
						clock(count); //
						if (count > 0) {
							$promise = $interval(function(borrowId){
							count --;
							if (count >= 0){
								clock(count);
							}
			...
			return {
				...
				link: function(scope, elm, attrs) {
					...
					$navs.on('click', function() {
						...
						if (attrs.switchAction) {
							$parse(attrs.switchAction)(scope, {
								index: targetIndex,
								value: this.value
							});
							scope.$apply();
